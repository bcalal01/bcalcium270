...
const vacation = document.getElementById('vacationButton');
...
const entries = [{ppl: [1], data: ["September 12", "7:30 PM", "Not busy at all"]},
                ...
                {ppl: [6], data: ["November 19", "6:00 PM", "surprisingly not that busy"]},
...
start.onclick = function () {
    start.innerHTML = "next shift"
    started = true;
    addDay();
}

vacation.onclick = function () {
    goVacation();
    vacation.style.display = "none"
    start.style.display = "block"
    start.innerHTML = "back to work"
    breaktime = false;
}
...
function goVacation() {
    background(255);
    fill(255);
    image(images[curr_image % 6], 0, 0, 600, 600);
    makeMe();
    curr_image++;
}

document.addEventListener("keydown", function(event) {
    if ((event.key == ' ' || event.code == 'Space') && started && !breaktime) {
        addDay();
        start.innerHTML = "next shift"
    }
  })
...
function addDay() {
    ...
    //makes the room
    makeRoom()
    if (entry % entries.length == entries.length - 1) {
...
function setup () {
    ...
    images[0] = loadImage('images/vac0.jpg');
    images[1] = loadImage('images/vac1.jpg');
    images[2] = loadImage('images/vac2.jpg');
    images[3] = loadImage('images/vac3.jpg');
    images[4] = loadImage('images/vac4.jpg');
    images[5] = loadImage('images/vac5.jpg');
    textSize(25);
    stroke(0);
    makeJCC();
}
...
function makeText() {
    fill(0)
    let entryData = entries[entry % entries.length].data;
    text(entryData[0], 10, 27)
    text(entryData[1], 10, 57)
    text(entryData[2], 10, 87)
}
